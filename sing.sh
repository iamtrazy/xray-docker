...
SING_FILE="sing-box-1.7.0-alpha.6-linux-${ARCH}.tar.gz"


echo "Downloading binary file: ${SING_FILE}"
wget -O /tmp/sing.tar.gz https://github.com/SagerNet/sing-box/releases/download/v1.7.0-alpha.6/${SING_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "Error: Failed to download binary file: ${SING_FILE}" && exit 1
fi
echo "Download binary file: ${SING_FILE} completed"

tar xvf /tmp/sing.tar.gz sing-box-1.7.0-alpha.6-linux-${ARCH}/sing-box -C /tmp --strip-components=1
